...
> **装完插件后先跑一次 `/obscura-fetch install`。** 插件本体只含指令与一个安装脚本——Obscura 引擎二进制（约 160MB）会下载到 `~/.obscura/bin/`，所有 agent 共用一份，且不随插件更新被清理。随时重跑该命令（或 `-Force`）升级。

#### 命令

| 命令 | 功能 |
|------|------|
| `/obscura-fetch help` | 快速入门指南 |
| `/obscura-fetch install` | 安装/升级 Obscura 引擎二进制 |
| `/obscura-fetch fetch <url>` | 抓取页面并输出渲染后 Markdown（含 JS 重页面） |
