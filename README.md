...
> **After installing the plugin, run `/obscura-fetch install` once.** The plugin ships instructions and one installer script — the Obscura engine binary (~160 MB) is downloaded to `~/.obscura/bin/`, shared by all your agents, and survives plugin updates. Upgrade any time by re-running the command (or `-Force`).

#### Commands

| Command | What it does |
|---------|-------------|
| `/obscura-fetch help` | Quick-start guide |
| `/obscura-fetch install` | Install/upgrade the Obscura engine binary |
| `/obscura-fetch fetch <url>` | Fetch a page as rendered Markdown (JS-heavy pages included) |
