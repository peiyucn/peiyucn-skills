...
|------|------|
| `help` | 显示快速入门指南 |
| `init` | 设置向导 — 选择语言、确定根路径、导入或从头开始，最后在笔记根目录写入 `AGENTS.md` 工作区清单 |
...
| `agentsmd` | 写入或刷新 `AGENTS.md` — 笔记根目录的工作区清单 |
...
命令：
  ...
  newtemplate    从任意分区提取模板
  archive        清理旧的笔记本、分区或页面
  agentsmd       写入或刷新 AGENTS.md — 你的工作区清单
...
AGENTS.md？
  未来的每个会话（任何 agent）动笔记前都会先读它。
  init 会自动写入；agentsmd 随时可写入或刷新，并询问要包含哪些板块。
  文件随时可以手改。
...
### Step 3 — 完成：写入 AGENTS.md

设置以运行 [`agentsmd` 流程](#agentsmd--写入或刷新-agentsmd)收尾，由它写入 `{notes_root}/AGENTS.md` — 一份把本次 init 决定沉淀下来的持久记录，让**未来的任何会话、任何 agent 都从同一份上下文起步**（跑 init 的这个会话不会是最后一个碰这些笔记的）。

从 init 委托时：

- **跳过它的笔记根目录与语言确认** — `{notes_root}` 已在（Step 1）确定、语言已选定（Step 0），两者直接复用。
- 把 init 的回答映射进清单：语言 → **初始化回顾 · 语言**；模式与结果 → **初始化回顾 · OneNote 导入历史**（"于 {日期} 导入 {N} 页" 或 "于 {日期} 从头开始"）。
- 运行 [`agentsmd` Step 3](#agentsmd--写入或刷新-agentsmd) 的内容选择问题，**「初始化回顾」默认勾选**（其答案来自 init，不重复提问）。已有文件检查与确认语与 `agentsmd` Step 2 / Step 5 完全一致。
...
## `agentsmd` — 写入或刷新 AGENTS.md

写入或刷新 `{notes_root}/AGENTS.md` — 记录本笔记工作区管理方式的工作区清单，让**未来的任何会话、任何 agent 都从同一份上下文起步**。同时作为 init 的收尾步骤运行（见 [init Step 3](#step-3--完成写入-agentsmd)）。

固定不变项 — 始终存在、不可选择：清单写明本工作区由 **note2md skill 管理**，并记录**使用 git 做版本管理**（若笔记根目录已经是或应当是 git 仓库）。用户选择包含哪些可选块。

### 步骤 1 — 确定笔记根目录

若会话已确定 `{notes_root}`（例如从 init 委托而来，或本会话早前已确认），直接复用。否则询问：
...
不扫描、不猜测。若所选目录还没有 `.note2md/`，提示一次："这个目录看起来尚未初始化 — 运行 `init` 可完成完整设置。"然后继续（清单本身可独立生效）。

**确定清单语言** — 文件写入所用的语言是一项持久决定，在 init 时一次性确定；各次运行之间不得漂移。按以下顺序确定：

1. 已存在的 `{notes_root}/AGENTS.md` 里有 **Language** 字段 → 用它。
2. 本会话跑过 init → 用 init 时选定的语言。
3. 两者都没有 → 现在问：`问题: "Select your language / 选择语言：" 选项: "中文" | "English"` — 复用 init 的 Step 0 问题。

确定下来的语言支配步骤 4 的渲染，并记录在清单的 **Language** 行里。

### 步骤 2 — 已有文件检查

若 `{notes_root}/AGENTS.md` 已存在，先读再判断：

- 有 `<!-- Generated by note2md -->` 标记 → 可刷新：继续（新内容整体替换）。
- 无标记 → **用户自有文件：不覆盖。** 告知用户，并提出可以把所选块**追加合并**进去 — 仅在用户明确同意后进行，合并结束时在文件末尾补上标记。用户拒绝则到此为止。

### 步骤 3 — 选择内容

固定块始终包含（在此列出是为了让用户知道会写入什么）：

1. **工作区** — 笔记根目录、笔记本 → 分区 → 页面层级、`.note2md/` 内部区规则（archive 默认不读，除非用户要求搜索归档）。
2. **note2md skill** — 本工作区由 note2md skill 管理；命令流程在 skill 里，不在此文件。
3. **版本管理** — 笔记用 git 做版本管理；未经用户同意绝不 commit；若笔记根目录还不是 git 仓库，本命令可顺带提供 `git init` + 基线 `.gitignore`（`.note2md/import/`、`.note2md/archive/`）。

可选块 — 一个多选问题：

```
问题: "AGENTS.md 要包含哪些板块？"
选项（多选）:
  - "初始化回顾 — 语言、导入历史"（从 init 委托时默认勾选）
  - "模板习惯 — 各笔记本/分区偏好使用的模板"
  - "敏感信息策略 — securecheck 应标记或跳过什么"
  - "自定义约定 — 你自己的长期规则"
  - "不要 — 只要固定块"
```

每个选中的块单独提问（自由文本，一次一问；跳过 = 记录"未记录"）：

| 板块 | 问题 |
|------|------|
| 初始化回顾 | 不提问 — 由 init 的回答填充（语言、OneNote 导入或从头开始）。独立运行且本会话没有 init 上下文时：问"这个库是从 OneNote 导入的吗？什么时候？" |
| 模板习惯 | "有模板使用习惯吗？例如「会议纪要固定用 meeting 模板、中文」" |
| 敏感信息策略 | "securecheck 要标记或跳过什么？例如「PRJ-XXXX 项目代号一律标记」「财务笔记本不扫描」" |
| 自定义约定 | "还有其他要 agent 长期遵守的约定吗？" |

### 步骤 4 — 写入

- 内容全部使用**已确定的清单语言**（步骤 1 的语言规则 — 以 init 最初的选定为准，绝不从会话重新推导）。唯一保持原样的固定字符串是两行标记注释 — 它们必须逐字保留，供未来的运行识别此文件。下方代码块展示英文形态；其余语言的每一行都以已确定的语言呈现：
  ...
  ## Init recap (standing orders for every session)          ← 仅选中时
  ...
  ## Template habits                                          ← 仅选中时
  - {用户描述的内容，或 "none recorded — pick the closest shipped template, ask when unsure"}

  ## Sensitive-info policy                                    ← 仅选中时
  - {securecheck 应标记或跳过的内容，或 "none recorded — use securecheck defaults"}

  ## Custom conventions                                       ← 仅选中时
  - {每条自由文本回答一个要点}
...
- 刷新已有生成文件 → 整体替换。合并进用户自有文件（仅限明确同意后）→ 在 `<!-- note2md -->` 注释头下追加所选板块，并在末尾补标记。
- 若「版本管理」记录的是*还不是 git 仓库*且用户同意 `git init`：现在执行（基线 `.gitignore` 缺失则一并创建），写入前更新该行。

### 步骤 5 — 确认

```
完成：AGENTS.md 已写入 {notes_root}/AGENTS.md（{N} 个板块）。未来每个会话都从这个文件起步。
```
...
> **AGENTS.md 轻提示：** 若 `{notes_root}/AGENTS.md` 不存在，在最终回复里加一句提示（用用户的语言，每次会话最多一次，无论跑的是哪个命令）："提示：你的笔记根目录还没有 AGENTS.md — 运行 `agentsmd` 即可生成（会把本工作区的管理方式记录下来，供未来每个会话使用）。"非阻断；命令照常继续。
...
`{notes_root}/AGENTS.md` 是工作区清单：一份记录本笔记工作区管理方式的持久文件，让**未来的任何会话、任何 agent 都从同一份上下文起步**。由 [`agentsmd` 命令](#agentsmd--写入或刷新-agentsmd)写入（并作为 init 的收尾步骤自动运行）。命令流程只存在于本 SKILL.md，**绝不**复制进 AGENTS.md — 清单只放数据与长期决定，每个会话把它们当作长期指令来读。

* **标记：** 带 `<!-- Generated by note2md -->` 标记的文件归本流程所有，整体刷新；不带标记的是用户自有文件 — 绝不覆盖。合并进用户自有文件仅在用户明确同意后进行。用户可以随时编辑该文件 — 除了须保留标记块，用户的修改永远优先。
* **内容范围：** 固定块（工作区规则、note2md skill 依赖、git 版本管理）+ `agentsmd` 步骤 3 中选中的可选块（初始化回顾、模板习惯、敏感信息策略、自定义约定）。不放命令表、不放流程。
* **轻提示：** 当某个命令会话发现 `{notes_root}` 下没有 `AGENTS.md` 时，agent 在最终回复里附一句提示（每次会话最多一次，非阻断），建议运行 `agentsmd`。
