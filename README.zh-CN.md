...
|------|------|
| `/note2md help` | 快速入门指南 |
| `/note2md init` | 初始化 — 选择语言、确定笔记目录、导入 OneNote 或从头开始，最后在笔记根目录写入 `AGENTS.md` 工作区清单 |
...
| `/note2md agentsmd` | 写入或刷新 `AGENTS.md` — 笔记根目录的工作区清单 |
...
每次 `/note2md init` 收尾时都会在笔记根目录写入一份 `AGENTS.md` — 记录工作区管理方式的清单：工作区规则、note2md skill 依赖、git 版本管理，外加你选的可选板块（初始化回顾、模板习惯、敏感信息策略、自定义约定）。未来的任何会话、任何 agent 在动你的笔记前都会先读它，从同一份上下文起步。文件随时可以手改；不带 `<!-- Generated by note2md -->` 标记的文件视为手写，绝不会被覆盖。老笔记库还没有？随时运行 `/note2md agentsmd` 补建或刷新。
...
> **装完插件后先跑一次 `/obscura-scrape install`。** 插件本体只含指令与一个安装脚本——Obscura 引擎二进制（约 160MB）会下载到 `~/.obscura/bin/`，所有 agent 共用一份，且不随插件更新被清理。随时重跑该命令（或 `-Force`）升级。

#### 命令

| 命令 | 功能 |
|------|------|
| `/obscura-scrape help` | 快速入门指南 |
| `/obscura-scrape install` | 安装/升级 Obscura 引擎二进制 |
| `/obscura-scrape fetch <url>` | 抓取页面并输出渲染后 Markdown（含 JS 重页面） |
